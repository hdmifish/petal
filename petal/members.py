...
class Members(object):
    def __init__(self, client):
        ...
        try:
            ...
            response = urllib2.urlopen("https://raw.githubusercontent.com/" +
                                       "hdmifish/petal/master/" +
                                       "example_members.json").read()
            self.doc = json.loads(response.decode('utf-8'))
            log.warn("members.json was missing, so I created one using the "
                     "default on github")
            ...
            if self.doc is None:
                ...
                response = urllib2.urlopen("https://raw.githubusercontent" +
                                           ".com/hdmifish/petal/master/" +
                                           "example_members.json").read()
                self.doc = json.loads(response.decode('utf-8'))
                log.warn("members.json was missing, so I created one using" +
                         " the default on my github")
    ...
    def addMember(self, mem):
        if mem.id in self.doc:
            ...
            try:
                ...
                self.doc[mem.id]["joinedAt"] = [self.doc[mem.id]
                                                ["joinedAt"]].append(
                                                str(datetime.utcnow()))
            return False
        else:
            self.doc[mem.id] = {"name": mem.name, "aliases": [],
                                "memberAt": "null", "leftAt": "null",
                                "messageCount": 0,
                                "lastOnline": str(datetime.utcnow()),
                                "osu": "null", "weather": "null",
                                "imgur": "aww",
                                "warnings": {}, "isBanned": False,
                                "tempBan": {}, "blockedChannels": [],
                                "trackedEvents": {}, "notes": {}}
            log.info(mem.name + " ID: " + mem.id +
    ...
    async def tempBan(self, member, author, reason, Days):
        if member.id not in self.doc:
            ...
            self.doc[member.id]["tempBan"][str(len(self.doc[member.id]
                                           ["tempBan"]))] = (
                    {"server": member.server.id, "active": True,
                     "date": str(datetime.utcnow()),
                     "expires": str(datetime.utcnow() + timedelta(days=Days)),
                     "issuer": author.name + " ({})".format(author.id),
                     "reason": reason})
